...
test.describe('Regression: full business flow', () => {
  ...
  test('7 - Scanning task papelito marks it done and achieves reward', async () => {
    ...
    for (const reward of pendingRewards) {
      ...
      if (!esComun && reward.usuario_id) q += `&usuario_id=eq.${reward.usuario_id}`;
      const tasks = await supabaseRest('GET', q);
      let points = 0;
      for (const t of tasks) points += t.fecha_escaneo ? t.puntos_ok : t.puntos_ko;
      if (points >= def.puntos_canjear) {
        await supabaseRest('PATCH', `papelito_recompensas?id=eq.${reward.id}`, { conseguido: true });
      }
    }
